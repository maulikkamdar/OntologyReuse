...
umlsontologiesFile = open(sys.argv[2])
umlsontologies = umlsontologiesFile.readlines()
umlsontologies = [x.strip().lower() for x in umlsontologies]

ontoTermsFile.write("Term URI\tOntology\tTerm Label\tOntology URI\tSubclassStr\n")
ontologyTerms = {}
previousTerm = ""
for root, dirs, files in os.walk(u"ontologies/"):
    for file in files:
        ...
        for i in range(len(ontologyLines)):
            ...
            isUmls = False
            for k in ontologyParts:
                if k.lower() in umlsontologies:
                    isUmls = True
                    break
            if isUmls:
                continue
            ...
            if not ontologyTriples[0] == previousTerm:
                if not previousTerm == "":
                    if ontologyTerms[previousTerm]["type"] == "class" and ontologyTerms[previousTerm]["termCui"] == "" and ontologyTerms[previousTerm]["termTui"] == "":
                        ontoTermsFile.write(previousTerm[1:len(previousTerm)-1] + "\t" + ontologyTerms[previousTerm]["ontology"]+ "\t" + ontologyTerms[previousTerm]["termLabel"] + "\t" + ontologyTerms[previousTerm]["ontologyUri"] + "\t" + ontologyTerms[previousTerm]["subClassOf"] + "\n")
                        #print previousTerm
                    else:
                        print previousTerm
